...
Please make sure that Qbot has the right to access this folder or change the folder path by providing the 'APP_DATA_PATH' env variable.
This is a fatal error, process will exit.`
  ...
  await Botpress.start({ modules: loadedModules.map(m => m.entryPoint) }).catch(err => {
    logger.attachError(err).error('Error starting Qbot')
  ...
  logger.info(`Qbot is listening at: ${process.LOCAL_URL}`)
  logger.info(`Qbot is exposed at: ${process.EXTERNAL_URL}`)
}
