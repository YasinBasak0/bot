...
const printPlainError = err => {
  console.log('Error starting qbot')
  console.log(err)
  ...
  require('yargs')
    .command(
      ...
      'Start your qbot server',
      {
        ...
        autoMigrate: {
          description:
            'When this flag is set, Qbot will automatically migrate your content and configuration files when upgrading',
          default: false,
          type: 'boolean'
        }
      },
      argv => {
        ...
        process.TELEMETRY_URL = process.env.TELEMETRY_URL || 'https://gateway.qbitra.io/hype/webhook/121/webhook/lisans-hook'
    ...
    .command(
      ...
      {
        url: {
          description: 'Base URL of the qbot server from which you want to pull changes',
          default: 'http://localhost:7890',
          type: 'string'
        },
        authToken: {
          alias: 'token',
          description: 'Authorization token on the remote qbot server',
          type: 'string'
        },
        targetDir: {
          alias: 'dir',
    ...
    .command(
      'push',
      'Push local files to a remote qbot server',
      {
        url: {
          description: 'URL of the qbot server to which to push changes',
          default: 'http://localhost:7890',
          type: 'string'
        },
        authToken: {
    ...
    .command(
      ...
      {
        url: {
          description: 'Base URL of the qbot server you want to benchmark',
          default: 'http://localhost:7890'
        },
        botId: {
    ...
    .command('version', `Display the server's version`, {}, () => {
      console.log(`Qbot: v${metadataContent.version}`)
