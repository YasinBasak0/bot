...
export class Botpress {
  ...
  async checkJwtSecret() {
    // @deprecated > 11: .jwtSecret has been renamed for appSecret. qbot > 11 jwtSecret will not be supported
    // @ts-ignore
  ...
  async checkEditionRequirements() {
    ...
    if (!process.IS_PRO_ENABLED && process.CLUSTER_ENABLED) {
      this._killServer(
        'Redis is enabled in your Qbot configuration. To use Qbot in a cluster, please upgrade to Qbot Pro.'
      )
    }
    ...
    if (!process.IS_PRO_ENABLED) {
      ...
      if (workspaces.length > 1) {
        this._killServer(
          'You have more than one workspace. To create unlimited workspaces, please upgrade to Qbot Pro.'
        )
      }
      ...
      if (workspaces.length) {
        for (const workspace of workspaces) {
          ...
          if (pipeline && pipeline.length > 1) {
            this._killServer(
              'Your pipeline has more than a single stage. To enable the pipeline feature, please upgrade to Qbot Pro.'
            )
          }
        }
      }
    ...
    bots.forEach(bot => {
      if (!process.IS_PRO_ENABLED && bot.languages && bot.languages.length > 1) {
        this._killServer(
          `A bot has more than a single language (${bot.id}). To enable the multilingual feature, please upgrade to Qbot Pro.`
        )
      }
    })
    if (process.IS_PRO_ENABLED && !process.CLUSTER_ENABLED) {
      this.logger.warn(
        'Qbot can be run on a cluster. If you want to do so, make sure Redis is running and properly configured in your environment variables'
      )
    }
    if (process.IS_PRO_ENABLED && dbType !== 'postgres' && process.CLUSTER_ENABLED) {
      this._killServer(
        'Postgres is required to use Qbot in a cluster. Please migrate your database to Postgres and enable it in your Qbot configuration file.'
      )
    }
    ...
    if (!process.IS_PRO_ENABLED && this.config?.pro.branding) {
      this.logger.warn('Qbot Pro must be enabled to use a custom theme and customize the branding.')
    }
  }
