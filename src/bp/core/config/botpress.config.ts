...
export type BotpressConfig = {
  ...
  httpServer: {
    ...
    port: number
    /**
     * There are three external URLs that Qbot calls: https://license.qbot.com.tr, https://duckling.qbot.com.tr and https://lang-01.qbot.com.tr
  ...
  pro: {
     ...
     * You can purchase a license on https://qbot.com.tr/
  ...
  /**
   * An array of e-mails of users which will have root access to Qbot (manage users, server settings)
   * @example: [admin@qbot.com.tr]
   */
  superAdmins: UniqueUser[]
  /**
   * When enabled, Qbot collects anonymous data about the bot's usage
   * @default true
   */
  ...
  /**
   * An array of categories in which a bot can be created in your qbot installation. Good for platform use-cases.
   * @example: ['customer service', 'e-commerce', 'etc']
   * @default []
   */
  botCategories: string[]
  /**
   * When this option is enabled, Super Admins are able to reboot the Qbot Server via the Admin UI
  ...
  jwtToken: {
    /**
     * The duration for which the token granting access to manage Qbot will be active.
     * @default 6h
     */
    duration: string
    /**
  ...
  /**
   * Displays the "Powered by Qbot" under the webchat.
  ...
  /**
   * By adding this, you'll make possible to translate a bot in more languages than those supported by your qbot language server
   * Warning: This means that Qbot NLU won't be working properly and you'll need to handle NLU on your own with a **beforeIncoming** Hook.
...
export interface AuthStrategy {
  ...
  /**
   * Set a label to display to users instead of the ID (ex: Qbot SSO)
   */
  label?: string
  /**
  ...
  /**
   * Maps the values returned by your provider to Qbot user parameters.
   ...
   * any user able to sign in using your SAML provider will create automatically an account on Qbot.
   * @default false
   */
...
export interface AuthStrategySaml {
   ...
   * @default http://localhost:7890/admin/login-callback
   */
  callbackUrl: string
  /**
   * The callback url is called by the SAML provider with the payload. The path provided here is relative to ${externalUrl}/admin
   * For example, if you use the default callback, it will be available at http://localhost:7890/admin/login-callback
...
export interface AuthStrategyOauth2 {
   ...
   * @default http://localhost:7890/api/v1/auth/login-callback/oauth2/myauth
   */
  callbackURL: string
  /*
   * Set this URL if your access token doesn't include user data. Qbot will query that URL to fetch user information
   * @example https://qbot.com.tr/userinfo
   */
  userInfoURL?: string
