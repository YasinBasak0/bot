declare namespace NodeJS {
  ...
  export interface Process {
    ...
    APP_SECRET: string
    /**
     * Path to the global APP DATA folder, shared across all installations of Qbot Server
    ...
    EXTERNAL_URL: string
    LOCAL_URL: string
    /** This is the subfolder where Qbot is located (ex: /qbot/). It is extracted from the external URL */
    ROOT_PATH: string
...
declare type BotpressEnvironmentVariables = {
  ...
  /** The URL exposed by Qbot to external users (eg: when displaying links) */
  readonly EXTERNAL_URL?: string

  /** Use this to override the hostname that qbot will listen on (by default it's localhost) - replaces httpServer.host */
  readonly BP_HOST?: string

  /** Change the port where qbot listens. Replaces the configuration of httpServer.port */
  ...
  /** When running qbot in production, some optimizations are applied*/
  readonly BP_PRODUCTION?: boolean

  /** Enable cluster mode */
  readonly CLUSTER_ENABLED?: boolean

  /** When you change the qbot executable, it will migrate data automatically if this is set */
   ...
   * @default https://license.qbot.com.tr
   */
  readonly BP_LICENSE_SERVER_HOST?: string

  /**
   * Set this to true if you're exposing qbot through a reverse proxy such as Nginx
   ...
   *  This values overwrites the value defined in the global qbot configuration
  ...
  /**
   * Truthy if running the official qbot docker image
   */
  ...
  /**
   * When set to true, qbot will not automatically restart on crash
   * @default false
   */
  ...
  /**
   * Prevents qbot from closing cleanly when an error is encountered.
