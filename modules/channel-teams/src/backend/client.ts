...
If you have a restricted app, you may need to specify the tenantId also.`
  ...
  async receiveIncomingEvent(req, res) {
    await this.adapter.processActivity(req, res, async turnContext => {
      ...
      var activityUsers = await this.adapter.getConversationMembers(turnContext)

      const threadId = conversationReference.conversation.id
      await this._setConversationRef(threadId, conversationReference)

      await this._sendIncomingEvent(activity, threadId, activityUsers)
    })
  }
  ...
  private _sendIncomingEvent = async (activity: Activity, threadId: string, userInformation: any) => {
    const { text, from, type, attachments } = activity
    const teamsInfo = userInformation[0]
    let downloadUrl = "0";
    if (attachments) {
      downloadUrl = attachments[0].content.downloadUrl;
    }
    await this.bp.events.sendEvent(
      this.bp.IO.Event({
        ...
        channel: 'teams',
        direction: 'incoming',
        payload: { text, teamsInfo, attachments, downloadUrl },
        preview: text,
        threadId,
        target: from.id,
  ...
  public async sendOutgoingEvent(event: sdk.IO.Event): Promise<void> {
    ...
    const eventdb = await this.bp.events.findEvents({ target: event.target })
    let threaddb = eventdb[0].threadId
    const ref = await this._getConversationRef(threaddb ?? event.threadId)
    if (!ref) {
      this.bp.logger.warn(
    ...
    } else if (msg.type === 'message' || msg.type === 'text') {
      //#url#
      var image
      const imgRegex = /#(.*)#/
      image = imgRegex.exec(msg.text as string)
      if (image) {
        let url = image[0].slice(1, -1)
        msg.text = (msg.text as string).replace(imgRegex, '')
        if (
          ...
          msg.attachments = [
            {
              ...
              contentType: _queryContentType(url),
              contentUrl: url
            }
          ]
        } else {
...
function _queryContentType(url) {
  let _types = 'image/png'
